...
Public Class Check_DuplicatesArcRvt
    Inherits AdvancedProdAction
    Public Sub New()
        Name = "RVT :: Check : Duplicates ARC (CQ-BIM)"
    End Sub
    ...
    Public Function MyMethod(Objects3D As MultipleElements) As ActionResult
        ...
        'ANALYSIS
        OutputTree = Audit_Library.Core.CompleteAuditDuplicates(tgmObjsList.ToList, outputList)

        Dim outputs = New List(Of Element) From {New MultipleElements(outputList, "DoublonsList", Nothing, ElementState.[New])}
        Dim res = New ActionResult(DateTime.Now.ToString("dd/MM/yy"), System.Drawing.Color.Green, outputs)
        Return res
    End Function
