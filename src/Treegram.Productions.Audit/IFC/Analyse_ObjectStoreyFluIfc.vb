...
Public Class Analyse_ObjectStoreyFluIfc
    Inherits AdvancedProdAction
    Public Sub New()
        Name = "IFC :: AUDIT : Storey Analysis"
    End Sub
    ...
    Public Overrides Sub CreateGuiInputTree()
        ...
        '------Fluides
        Dim fluidNode = ifcNode.SmartAddNode("object.type", "ifc", Nothing, Nothing, "Fluide",, True)
        Dim fluidtypesList As New List(Of String) From {"IfcCovering", "IfcDistributionPort", "IfcFlowController", "IfcFlowFitting", "IfcFlowSegment", "IfcFlowTerminal", "IfcSystem"}
        fluidtypesList.Sort()

        Dim fluidNodesList As New List(Of Node)
        For Each typeSt In fluidtypesList
            fluidNodesList.Add(ifcNode.SmartAddNode("object.type", typeSt))
        Next

        SelectYourSetsOfInput.Add("Objects3D", fluidNodesList)
